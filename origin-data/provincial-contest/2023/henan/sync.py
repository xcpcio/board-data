    ...
    for item in res_list:
        ...
        for team in item['commonRankings']['commonRankings']:
            if 'studentUser' in team['user'].keys():
                ...
                team_key = team['user']['studentUser']['name'].strip()
                if team_key not in team_info.keys():
                    if team_key.startswith("*"):
                        team_key = team_key[1:]

                    name = team_key.split('-')[1]
                    school = team_key.split('-')[0]
